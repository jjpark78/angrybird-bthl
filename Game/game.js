...
var GameLayer = cc.Layer.extend({
  ...
  slingRubber2: null,
  slingRubber3: null,
  party: null, // party image by jungkun
  getTexture: function (name) {
    // file extension by jungkun
    var ext = name === "party" ? ".gif" : ".png";
    // return cc.TextureCache.getInstance().addImage("sprites/" + name + ".png");
    return cc.TextureCache.getInstance().addImage("sprites/" + name + ext);
  },
  ...
  init: function () {
    ...
    (tempEnemyObj.userData = new BodyUserData(GameObjectRoll.Enemy, 40)),
      (tempEnemyObj.x = tempBoxObj.x = 1231.5),
      (tempEnemyObj.y = 771),
      (tempBoxObj.y = tempEnemyObj.y - 20);
    this.addObject(tempEnemyObj), this.addObject(tempBoxObj);
    // enemy2
    (tempEnemyObj.userData = new BodyUserData(GameObjectRoll.Enemy, 40)),
      (tempEnemyObj.x = tempBoxObj.x = 1431.5),
      (tempEnemyObj.y = 771),
      (tempBoxObj.y = tempEnemyObj.y - 20);
    this.addObject(tempEnemyObj), this.addObject(tempBoxObj);
    // enemy3
    (tempEnemyObj.userData = new BodyUserData(GameObjectRoll.Enemy, 40)),
      (tempEnemyObj.x = tempBoxObj.x = 1631.5),
      (tempEnemyObj.y = 771),
      (tempBoxObj.y = tempEnemyObj.y - 20);
    this.addObject(tempEnemyObj), this.addObject(tempBoxObj);
    // enemy4
